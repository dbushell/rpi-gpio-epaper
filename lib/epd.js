...
class EPD {
  ...
  init() {
    return new Promise(async (resolve, reject) => {
      try {
        // setup pins
        await GPIOWrapper.setup(PINS.RST, GPIOWrapper.DIR_OUT);
        await GPIOWrapper.setup(PINS.DC, GPIOWrapper.DIR_OUT);
        await GPIOWrapper.setup(PINS.CS, GPIOWrapper.DIR_OUT);
        await GPIOWrapper.setup(PINS.BUSY, GPIOWrapper.DIR_IN);
        // open spi
        await this.spi.open();
        // setup display mode
  ...
  writeCommand(command, para = false) {
    return new Promise(async (resolve, reject) => {
      ...
      // await GPIOWrapper.output(PINS.CS, false); // not required?
      await GPIOWrapper.output(PINS.DC, false);
      await this.spi.write([command]);
      if (para !== false) {
        await GPIOWrapper.output(PINS.DC, true);
        await this.spi.write([para]);
      }
      // await GPIOWrapper.output(PINS.CS, true); // not required?
      resolve();
    });
  }
  ...
  initDisplay() {
    return new Promise(async (resolve, reject) => {
      try {
        // await GPIOWrapper.output(PINS.CS, false);
        await GPIOWrapper.output(PINS.RST, false);
        await sleep(100);
        await GPIOWrapper.output(PINS.RST, true);
        await sleep(100);
