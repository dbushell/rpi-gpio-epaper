...
class EPD {
  ...
  init() {
    return new Promise(async (resolve, reject) => {
      try {
        // setup pins
        await gpio.setup(PINS.RST, gpio.GPIO.DIR_OUT);
        await gpio.setup(PINS.DC, gpio.GPIO.DIR_OUT);
        await gpio.setup(PINS.CS, gpio.GPIO.DIR_OUT);
        await gpio.setup(PINS.BUSY, gpio.GPIO.DIR_IN);
        // open spi
        await this.spi.open();
        // setup display mode
  ...
  writeCommand(command, para = false) {
    return new Promise(async (resolve, reject) => {
      ...
      // await gpio.output(PINS.CS, false); // not required?
      await gpio.output(PINS.DC, false);
      await this.spi.write([command]);
      if (para !== false) {
        await gpio.output(PINS.DC, true);
        await this.spi.write([para]);
      }
      // await gpio.output(PINS.CS, true); // not required?
      resolve();
    });
  }
  ...
  initDisplay() {
    return new Promise(async (resolve, reject) => {
      try {
        // await gpio.output(PINS.CS, false);
        await gpio.output(PINS.RST, false);
        await sleep(100);
        await gpio.output(PINS.RST, true);
        await sleep(100);
